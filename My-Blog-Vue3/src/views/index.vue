...
const setArticleTotal = (value) => store.setArticleTotal(value);
const setNewArticles = (value) => store.setNewArticles(value);

const printerInfo = ref("咦，这个地方是用来干什么的？");
...
// const style = ref("");
// const size = ref(210);
const parentLoadingMark = ref(false);
...
const getArticles = async () => {
  try {
    ...
    if (!$common.isEmpty(res.result[0])) {
      ...
      articles.value = [...articles.value, ...res.result[0].records];
      pagination.value.total = res.result[0].total;
      setArticleTotal(res.result[0].total);
...
const getRecommendArticles = async () => {
  ...
  try {
    ...
    if (!$common.isEmpty(res.result[0])) {
      const formatRecommendArticles = res.result[0].records;
